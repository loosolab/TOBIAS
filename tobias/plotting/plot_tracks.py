...
def run_tracks(args):
	...
	with open(args.regions) as f:
		for line in f:
			...
			pa.config["output_filename"] = os.path.join(args.outdir, prefix, prefix + ".pdf")
			...
			if len(data_from_gtf["for_transcript"]) > args.max_transcripts:
				data_from_gtf["for_transcript"] = data_from_gtf["for_transcript"][:args.max_transcripts]
